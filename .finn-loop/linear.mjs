...
function loadEnv(path) {
  ...
  for (const raw of readFileSync(path, "utf8").split(/\r?\n/)) {
    ...
    if (!match || raw.trimStart().startsWith("#") || process.env[match[1]]) {
      continue;
    }
...
if (!existsSync(configPath)) {
  throw new Error("Missing .finn-loop/config.json");
}
...
const issueFields = `
  ...
  relations {
    nodes {
      id
      type
      relatedIssue { id identifier title state { type name } }
    }
  }
`;
...
async function team() {
  ...
  if (!found) {
    throw new Error(`Linear team ${config.linearTeamKey} not found`);
  }
  return found;
}
...
async function labelId(name) {
  ...
  const label = currentTeam.labels.nodes.find(
    (item) => item.name.toLowerCase() === name.toLowerCase(),
  );
  if (!label) {
    throw new Error(
      `Linear label "${name}" not found on team ${currentTeam.key}`,
    );
  }
  return label.id;
}
...
if (command === "viewer") {
  ...
  result = data.issues.nodes
    ...
    .filter((item) =>
      item.labels.nodes.some((label) => label.name === ready),
    )
    .filter((item) =>
      !item.labels.nodes.some((label) => label.name === blocked),
    )
    .sort(
      (a, b) =>
  ...
  const descriptionPath = arg("--description-file");
  if (!title) throw new Error("--title is required");
  if (!descriptionPath) throw new Error("--description-file is required");
  ...
  const currentTeam = await team();
  const state = currentTeam.states.nodes.find(
  ...
  const state =
    currentTeam.states.nodes.find(
      (item) => item.name.toLowerCase() === preferred,
    ) ?? currentTeam.states.nodes.find((item) => item.type === "started");
  if (!state) {
    throw new Error("No started Linear workflow state found");
  }
  result = (
    await gql(
      `mutation($id: String!, $input: IssueUpdateInput!) {
  ...
  result = (
    await gql(
      ...
      }`,
      {
        ...
        input: {
          assigneeId: null,
          labelIds: [...new Set([...existing, blockedId])],
        },
      },
    )
  ).issueUpdate;
  ...
  throw new Error(
    "Commands: viewer, team, ready, issue, create-issue, claim, comment, move, block",
  );
}
